...
| `streaming/shared.ts` | Shared `StreamState` (pending text/reasoning flush, tool-call emission and dedup), retry step-down (`getRetryReasoningEffort`, `REASONING_EFFORT_FALLBACK_ORDER`), pending-call emission, attempt snapshots, and truncation reporting. Used by all three streaming parsers. |
