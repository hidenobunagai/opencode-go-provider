...
jest.mock('vscode', () => ({
  ...
  LanguageModelToolCallPart: class { constructor(public callId: string, public name: string, public input: Record<string, unknown>) {} },
...
describe('OcGoChatModelProvider', () => {
  ...
  it('provideLanguageModelChatInformation returns fallback models when cache is empty', async () => {
    ...
    const infos = await provider.provideLanguageModelChatInformation({ silent: true } as any, token as any);
  ...
  it('provideLanguageModelChatInformation returns empty array on cancellation', async () => {
    const token = { isCancellationRequested: true, onCancellationRequested: jest.fn(() => ({ dispose: jest.fn() })) };
    const infos = await provider.provideLanguageModelChatInformation({ silent: true } as any, token as any);
  ...
  it('throws when message exceeds token limit', async () => {
    ...
    await expect(
      provider.provideLanguageModelChatResponse(
        ...
        [{ role: 1, content: [{ value: 'This is a very long message that exceeds the token limit' }] }] as any,
        { modelOptions: {} } as any,
        progress,
        token as any
      )
  ...
  it('prompts for an API key during chat and continues the request when one is provided', async () => {
    ...
    expect(progress.report).toHaveBeenCalledWith(expect.objectContaining({ value: 'Hello from OpenCode Go' }));
  });
  ...
  it('streams tool call parts', async () => {
    ...
    await provider.provideLanguageModelChatResponse(
      { id: 'kimi-k2.6', maxInputTokens: 100000, maxOutputTokens: 65536 } as any,
      [{ role: 1, content: [{ value: 'Hi' }] }] as any,
      { modelOptions: {}, tools: [{ name: 'get_weather', description: 'Get weather', inputSchema: {} }] } as any,
      progress,
      token as any
    );
  ...
  it('emits text that appears before a tool call in the same response', async () => {
    ...
    expect(progress.report.mock.calls[0][0]).toEqual(expect.objectContaining({ value: 'Let me check ' }));
    expect(progress.report.mock.calls[1][0]).toEqual(expect.objectContaining({ callId: 'call_1', name: 'get_weather' }));
  });
  ...
  it('emits text that appears after a tool call in the same response', async () => {
    ...
    expect(progress.report.mock.calls).toHaveLength(2);
    expect(progress.report.mock.calls[0][0]).toEqual(expect.objectContaining({ callId: 'call_1', name: 'get_weather' }));
    expect(progress.report.mock.calls[1][0]).toEqual(expect.objectContaining({ value: 'Now I have the weather.' }));
  });
  ...
  it('emits a tool call parsed from text-embedded control tokens', async () => {
    ...
    const mockStream = async function* () {
      yield {
        choices: [{
          delta: {
            content: '<|tool_call_begin|>read_file<|tool_call_argument_begin|>{"filePath":"/tmp/example.md"}<|tool_call_end|>',
          },
        }],
      };
    };
  ...
  it('preserves text order around a text-embedded tool call', async () => {
    ...
    const mockStream = async function* () {
      yield {
        choices: [{
          delta: {
            content: 'Before <|tool_call_begin|>read_file<|tool_call_argument_begin|>{"filePath":"/tmp/example.md"}<|tool_call_end|> after',
          },
        }],
      };
    };
    ...
    expect(progress.report.mock.calls[1][0]).toEqual(expect.objectContaining({ name: 'read_file' }));
  ...
  it('emits a tool call when text-embedded control tokens are split across chunks', async () => {
    ...
    const mockStream = async function* () {
      yield {
        choices: [{
          delta: {
            content: '<|tool_call_begin|>read_file<|tool_call_argument_begin|>{"filePath":"/tmp/exa',
          },
        }],
      };
      yield {
  ...
  it('allows the same tool call again after an intervening user message', async () => {
    ...
    await provider.provideLanguageModelChatResponse(
      ...
      [
        ...
        },
        {
          role: 1,
          content: [
            new (vscode as any).LanguageModelTextPart('Read that same line again.'),
          ],
        },
      ] as any,
      {
    ...
    const toolCallReports = progress.report.mock.calls.filter((c: any) => c[0]?.callId);
    expect(toolCallReports).toHaveLength(1);
    expect(toolCallReports[0][0]).toEqual(expect.objectContaining({ callId: 'read_file:1', name: 'read_file' }));
  });
      ...
      [
        {
          ...
          content: [
            ...
            new (vscode as any).LanguageModelToolCallPart('call_1', 'get_weather', { city: 'Tokyo' }),
          ],
        },
        {
          role: 1,
          content: [
            new (vscode as any).LanguageModelToolResultPart('call_1', [new (vscode as any).LanguageModelTextPart('Sunny, 25C')]),
          ],
        },
      ] as any,
      { modelOptions: {}, tools: [{ name: 'get_weather', description: 'Get weather', inputSchema: {} }] } as any,
