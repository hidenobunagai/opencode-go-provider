...
import { convertMessages, estimateTokens, estimateMessagesTokens, convertTools } from '../src/utils';
...
describe('convertMessages', () => {
  ...
  it('converts assistant text message', () => {
    const messages = [
      { role: vscode.LanguageModelChatMessageRole.Assistant, content: [new vscode.LanguageModelTextPart('Hi there')] },
    ];
    const result = convertMessages(messages as any);
    expect(result).toEqual<OcGoChatMessage[]>([{ role: 'assistant', content: 'Hi there', reasoning_content: ' ' }]);
  });

  it('converts system text message', () => {
    const messages = [
      { role: (vscode as any).LanguageModelChatMessageRole.System, content: [new vscode.LanguageModelTextPart('Be helpful')] },
    ];
    const result = convertMessages(messages as any);
...
describe('convertMessages with tools', () => {
    ...
    const messages = [
      {
        role: vscode.LanguageModelChatMessageRole.User,
        content: [
          new vscode.LanguageModelToolResultPart('call_1', [new vscode.LanguageModelTextPart('Sunny, 25C')]),
        ],
      },
    ];
    const result = convertMessages(messages as any);
  ...
  it('decodes base64 json data parts in tool results', () => {
    const jsonBytes = Buffer.from(JSON.stringify({ filePath: '/tmp/a.txt', content: 'hello' }), 'utf8');
    const messages = [
      {
        role: vscode.LanguageModelChatMessageRole.User,
        content: [
